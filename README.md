...
**requirements.txt** added (no Python deps needed; Vercel uses `package.json`).

**vercel.json** added for explicit build config.

### Quick Deploy:
1. `git add . && git commit -m "Add Vercel deploy" && git push origin main`
2. vercel.com → New Project → Import GitHub repo
3. Auto deploys! Live URL ready.
